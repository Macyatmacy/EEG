function [output_paths] = train_KNN(data_paths,feature_train_paths,feature_test_paths,save_folder_name,fast_knn)
...
for i = 2:length(data_paths)
    ...
    for f = 1:length(train_files)
        if contains(train_files(f).name,'.mat')
            ...
            output_KNN = optimized_KNN(train_file_path,test_file_path,fast_knn);
            save([save_folder_name,'\',train_files(f).name(1:end-4),'_KNN.mat'],'output_KNN');
        end
    end
end

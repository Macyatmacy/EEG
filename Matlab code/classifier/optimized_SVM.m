function [output_SVM] = optimized_SVM(train_file_path,test_file_path,fast_svm)
...
%rng(1);
if fast_svm
    [trainedClassifier, validationAccuracy] = svm_qua(train_x, train_y);
...
if fast_svm
    [trainedClassifier, validationAccuracy] = svm_gua(train_x, train_y);
    yfit = trainedClassifier.predictFcn(test_x);
    yfit = string(yfit);
    idx = find(test_y==yfit);

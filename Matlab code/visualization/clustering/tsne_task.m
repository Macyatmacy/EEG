function tsne_data_path = tsne_task(topfolder, epoched_paths,save_folder_name)
...
for i = 1:length(epoched_paths)
    ...
    for f = 1:length(files)
        if contains(files(f).name,'.mat')
            ...
            load(file_path);
            if f == 3
                label = append("hear chi");
            elseif f == 4
                label = append("hear eng");
            elseif f == 5
                label = append("inner chi");
            elseif f == 6
                label = append("inner eng");
            elseif f == 7
                label = append("speak chi");
            elseif f == 8
                label = append("speak eng");
            end
            file_path = [files(f).folder,'\',files(f).name];
